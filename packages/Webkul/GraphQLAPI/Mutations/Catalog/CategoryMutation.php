...
class CategoryMutation extends Controller
{
    ...
    public function store($rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            ...
            if ( isset($category->id)) {
                $this->uploadImage($category, $image_url);
    ...
    public function uploadImage($category, $image_url)
    {
        $category_path = 'category/' . $category->id . '/';
        $image_dir_path = storage_path('app/public/' . $category_path);
        if (! file_exists($image_dir_path)) {
            mkdir(storage_path('app/public/' . $category_path), 0777, true);
        }
        ...
        if ( isset($image_url) && $image_url) {
            ...
            if ( $valoidateImg ) {
                ...
                if ( file_exists($savePath) ) {
                    Storage::delete('/' . $category_path . $img_name);
                }

                file_put_contents($savePath, file_get_contents($image_url));

                $category->image = $category_path . $img_name;
                $category->save();
            }
        }
    }
